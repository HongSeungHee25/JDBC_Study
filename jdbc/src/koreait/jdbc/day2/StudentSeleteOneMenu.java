...
public class StudentSeleteOneMenu {
	...
	private static void selectOneStudent(Connection conn) {
		...
		System.out.print("조회할 학생 학번을 입력해주세요 >> ");
		...
		PreparedStatement ps = conn.prepareStatement(sql);
			...
			System.out.println("rs 객체의 구현 클래스 : "+rs.getClass().getName());
			// oracle.jdbc.driver.OracleResultSetImpl 클래스 객체로 만들어집니다.
			
			//rs.next() 데이터를 가져올 커서(위치)를 다음 행으로 이동합니다.
			//조회 결과 유무를 알려면 '제일 먼저 실행해야 할 메소드 - 조회 결과 첫번째 행으로 이동' 입니
			...
			System.err.println("데이터 조회에 문제가 생겼습니다. : "+e.getMessage());
