...
//java에서 지원하는 그래픽 UI 라이브러리 : awt -> swing -> javaFx
//필수적인것은 JFrame 을 상속하여 윈도우를 준비.
// 			ActionListener 인터페이스 구현을 해서 사용자가 일으키는 동작(이벤트)을 처리하는 메소드 재정의
public class D1GUiMain extends JFrame implements ActionListener{
	...
	public D1GUiMain() {
		//컴포넌트 배치를 위해서 레이아웃을 사용할 수 있으나, 여기서는 레이아웃 없이 모든 컴포넌트를 좌표로 지정.
	...
	public void actionPerformed(ActionEvent e) {  
							     //  ┗> 이벤트 : 사용자 동작 (버튼 클릭...) 컴포넌트를 e 변수가 참조
		if(e.getSource()==b1) {    //첫번째 버튼
			...
			dispose();				//윈도우 화면 해제
			System.exit(0);        //종료. 0아닌 값은 JVM 에게 비정상종료임을 전달.
		}
