...
public class D2JTableList extends JFrame {
	...
	public D2JTableList() {
		...
		list.add(new Word("aaa", "에이", 3, LocalDate.parse("2023-05-28")));
		String find="";
		...
		for(int i=0;i<list.size();i++) {
			...
			if(flag) {
				...
				data[1] = temp.getKorean();						//Word 객체의 한글을 배열로 저장
				data[2] = String.valueOf(temp.getLevel());		//Word 객체의 레벨을 배열로 저장
				data[3] = temp.getWday().toString();			//Word 객체의 날짜를 배열로 저장
